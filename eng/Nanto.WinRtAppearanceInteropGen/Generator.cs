...
public static class Generator
{
    ...
    public static void Generate(string packageRoot, string specPath, string outputDirectory) =>
        Generate(new GeneratorArguments(
            Path.GetFullPath(packageRoot),
            Path.GetFullPath(specPath),
            Path.GetFullPath(outputDirectory),
            VerifyDirectory: null));

    internal static void Generate(GeneratorArguments arguments)
    {
        ...
        GenerationManifest manifest = new()
        {
            ...
            [
                ...
                new("eng/Nanto.WinRtAppearanceInteropGen/winrt-appearance-spec.json", Hash(specBytes)),
            ],
            RuntimeClass = new(
            ...
            Output = new(
                "src/Nanto.Hosting.Windows/Interop/Generated/WinRtAppearanceInterop.g.cs",
                sourceBytes.Length,
                Hash(sourceBytes),
                "utf-8",
                "lf"),
