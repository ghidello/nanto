...
System appearance follows Microsoft's supported Win32 guidance through the target framework's generated `Windows.UI.ViewManagement.UISettings` projection, `ColorValuesChanged`, and `DWMWA_USE_IMMERSIVE_DARK_MODE`; it adds no WinUI, Windows App SDK, or package dependency, and the undocumented `AppsUseLightTheme` registry value is not a Nanto contract. The application-level WebView owner owns and deterministically releases the UISettings native reference and subscription. A shorter window registration is acquired after `HWND` creation, before WebView2 acquisition or visibility, and is released after controller cleanup but before native-window destruction.

The native frame and WebView2 profile use the same portable preference. Explicit mutations run on the owning STA, update the native frame before the profile, roll the frame back if the profile update fails, and publish `INantoApplicationHost.PreferredColorScheme` only after both succeed. `ColorValuesChanged` marshals to the STA and reapplies the frame only in `System`; explicit `Light` or `Dark` ignores later system notifications. WebView2 remains in its native `Auto` mode for `System`, so the SPA follows through `prefers-color-scheme` without a Nanto message. Fast tests cover preference resolution, projected UISettings activation/release, DWM attribute mutation, system-event behavior, detached-window races, acquisition failure, and zero ledger counts. Hidden production tests retain live Dark/Light/System SPA verification and add the appearance acquisition checkpoints to the fault/cancellation matrix.
