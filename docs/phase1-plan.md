...
| `tests/Nanto.Hosting.Windows.TestApp/Nanto.Hosting.Windows.TestApp.csproj` | `net10.0-windows10.0.19041.0` | References Core, Windows hosting, and TestProtocol; external process used by every native integration project. Its default framework-dependent mode produces a DLL, while self-contained CoreCLR and Native AOT modes produce executable apphosts. |
...
Pin `Microsoft.Web.WebView2` at `1.0.4129.50`, the generation-only `Microsoft.Windows.SDK.NET.Ref` at `10.0.19041.57`, `Microsoft.Windows.CsWin32` at `0.3.298`, and `Microsoft.Extensions.Logging.Abstractions` at `10.0.10` in central package management. Upgrade an interop input only through an explicit dependency review that includes deterministic regeneration, ABI verification, AOT/trim diagnostics, deployment inspection, and real-host integration tests. Do not add `Microsoft.Extensions.Hosting`, a DI container, OpenTelemetry SDK, or a UI framework in Phase 1.
...
Milestone 6 Batch 1 implements this mode contract through TestApp's scoped `Directory.Build.props`, before SDK output paths are established. An omitted `NantoBuildMode` selects `CoreClrFrameworkDependent`; an explicitly empty or unknown value fails before compilation. `NantoBuildMode` remains independent from `Configuration` and `TestScope`. The process runner launches `.dll` TestApp outputs through `dotnet` and `.exe` outputs directly, rejecting every other launch shape. Ordinary solution builds therefore exercise the framework-dependent DLL without publishing the later deployment modes. Concurrent deployment validation also confirms that the deadline started by the first stop request is the sole shutdown-timeout authority; teardown does not start a competing later deadline that can publish incomplete diagnostics.
...
Milestone 6 Batch 2 implements the self-contained lane. Its test-execution target safely recreates only the exact ignored publish, symbol, package, and evidence locations, publishes TestApp in `Release`, retains every project PDB separately, and removes transitive symbol copies from the deployable directory. The test-only inspector rejects reparse points and non-file entries, hashes a sorted inventory, reads bounded PE imports and managed assembly references without external tools, validates Windows Authenticode trust and Microsoft version metadata for `WebView2Loader.dll`, proves `Nanto.Hosting.Windows.dll` has no `WinRT.Runtime` reference, and creates a byte-deterministic ZIP with fixed entry timestamps. The untrimmed Windows self-contained runtime pack still copies its general `Microsoft.Windows.SDK.NET.dll` and `WinRT.Runtime.dll` assets; Nanto does not rewrite framework dependency manifests or trim this diagnostic CoreCLR mode merely to hide those unused platform files. Source-generated JSON records the SDK, TFM, RID, architecture, deployable and symbol inventories, raw/ZIP sizes and hashes, declared embedded-asset bytes, loader result, imports, and smoke results. `HostLifecycle`, `Navigation`, and `RendererRecovery` each launch the direct apphost, finish with zero active resources, release browser processes, and leave application storage deletable. Ordinary solution builds do not invoke this publication target. These measured sizes include TestApp's acceptance protocol and automation code and exclude the externally installed Evergreen WebView2 Runtime; they are engineering baselines, not advertised minimal-application sizes.

Milestone 6 Batch 3 implements the Native AOT lane with the same evidence contract and smoke scenarios. An application-imported build target owns loader selection at the final executable boundary: it follows `PublishAot`, direct-P/Invokes the logical `WebView2Loader` library and links the pinned package's `WebView2LoaderStatic.lib` for Native AOT, or copies the pinned dynamic loader for CoreCLR. `Nanto.Hosting.Windows` therefore does not guess the consuming application's mode or require propagation of the TestApp-only `NantoBuildMode`. TestApp imports this target explicitly; future SDK/package tooling must deliver the same target to consuming applications. The Native AOT deployable contains neither the static library nor `WebView2Loader.dll`, CoreCLR, hostfxr, managed dependency metadata, or PDBs. The inspector derives x64 architecture from the executable's COFF header, rejects a dynamic loader or CoreCLR import, retains managed analysis PDBs and the native linker PDB in separate symbol trees, verifies the Native AOT reachability map contains no `WinRT.Runtime`, and records the deterministic package and zero-ledger smoke results. The warning-as-error Release publish completes with full trimming and AOT analysis and no unexplained warnings. As with self-contained CoreCLR, the measured deployment includes acceptance-only TestApp code and excludes the Evergreen WebView2 Runtime.
...
Both offline interop generators and their shared integration test are members of `Nanto.slnx`. Ordinary `dotnet build` consumes only committed generated sources, and default `dotnet test` does not execute regeneration; `dotnet test -p:TestScope=All` includes deterministic regeneration and comparison. A developer may run the interop-generation project directly with `-p:TestScope=All` for a narrow check. The project is safe for unattended CI because it creates no native window, browser process, or desktop interaction.

`Nanto.WinRtAppearanceInteropGen` resolves only the centrally pinned `Microsoft.Windows.SDK.NET.Ref` 10.0.19041.57 targeting pack. Its reviewed specification selects `UISettings.GetColorValue` and `ColorValuesChanged`; generation validates the runtime/default-interface closure, complete same-interface prefix, layouts, event token, signatures, and closed callback IID before emitting a raw `IInspectable` vtable projection. The generated source and manifest use LF endings and are reproduced byte-for-byte beside the WebView2 projection. Production never references the comparison spike, the SDK C#/WinRT projection, or `WinRT.Runtime`.
...
`ApplicationIdentity` and `ValidatedApplicationOptions` have no public constructors. Hosts call `ApplicationIdentity.Parse` or, normally, `ValidatedApplicationOptions.Create`; validated options then call `CreateWebAssetPreparationContext(applicationRootDirectory)` with the already prepared absolute platform storage root and validated application-owned logger factory. This prevents a provider from deriving `%LOCALAPPDATA%` independently or pairing an application ID with an inconsistent key/root. Provider implementations can read the context's public properties and create loggers, but cannot construct the context or dispose its factory. A directory provider may ignore the application root, while the versioned provider publishes beneath it. Route and timestamp validation helpers remain internal implementation details.
...
- Raise `RendererFailed` before acting, and isolate exceptions thrown by application event handlers.
- Attempt at most one main-frame renderer `Reload` during a window lifetime, and only while the window is running and no close has been requested.
- Treat the following successful navigation completion as recovery. A synchronous reload failure, failed recovery navigation, repeated main-renderer failure, or close race requests ordinary window closure exactly once.
- Browser-process exit is not renderer recovery: report `Exited` without a recovery attempt and close normally. Because the native browser state is already gone, teardown releases Nanto's subscription, mapping, controller, and COM ownership without calling unavailable WebView2 mutation methods. Automatic whole-host recreation remains outside Phase 1.
- Treat frame renderer and runtime-recreated child-process failures as diagnostic-only events.

### Logging and resource ledger

Use source-generated `LoggerMessage` methods and never renumber or reuse an event ID. Reserve `1–99` for application lifecycle, `100–199` for window lifecycle, `200–299` for the dispatcher/UI thread, `300–399` for WebView/process/appearance, `400–499` for assets/storage, and `500–599` for teardown. Information records major lifecycle outcomes, Debug records acquisition/state detail, Warning records recovered anomalies, and Error records terminal, callback, deadline, and cleanup failures. Log only hash-only storage correlation IDs, bundle hashes, window IDs, bounded states and symbolic operations, HRESULT/native codes, counts, and elapsed durations. Never log application IDs, filesystem storage keys containing application-name segments, titles, routes, absolute or asset paths, web-message bodies, command payloads, cookies, local-storage values, or file contents. Attach exceptions only for application callbacks that Nanto swallows; framework, filesystem, navigation, COM, and native failures retain their complete exception for the caller while logs contain only type, stage, operation, and numeric code. The resource ledger remains authoritative for detailed ownership and is not replayed into logs.

The internal debug ledger tracks application hosts, UI threads, windows, native handles, COM objects, subscriptions, dispatcher items, asset leases, virtual-host mappings, and browser processes. Each lease receives a stable process-local ID. Acquisition and release are paired in the owning component. Counts remain available without retaining an unbounded history; TestApp explicitly enables the diagnostic ownership trace and serializes named acquisitions and releases in order alongside the initial, peak, and final snapshots. A successful or expected-failure scenario requires every owned count except the process's baseline OS handle count to return to zero. The external oracle verifies the documented dependency order within and across scopes: subscriptions and mapping precede controller/COM release; the controller precedes its parent `HWND`; window class follows the window; the application asset lease precedes the shared environment; and the UI thread stops last. When the browser process has already exited, the same logical ownership order applies even though unavailable native state cannot be explicitly mutated. Dispatcher items remain independently scoped queue operations and are instead required to be paired and zero at completion.
...
- One shared kill-on-close process group contains every child. The first measured attempt completed 397 clean processes in 45 minutes and projected roughly 62–64 minutes for all 550, so a 75-minute outer deadline now provides measured cleanup headroom. The deadline or first failure terminates the active tree and prevents further launches.
...
System appearance follows Microsoft's supported Win32 guidance through `Windows.UI.ViewManagement.UISettings`, `ColorValuesChanged`, and `DWMWA_USE_IMMERSIVE_DARK_MODE`. A pinned offline generator emits only the required raw WinRT ABI; the production assembly references neither the broad SDK C#/WinRT projection nor `WinRT.Runtime`, WinUI, or Windows App SDK, and the undocumented `AppsUseLightTheme` registry value is not a Nanto contract. The UI thread initializes and balances the Windows Runtime apartment for its complete lifetime. The application-level WebView owner owns and deterministically releases the UISettings interface, native callback, and subscription. A shorter window registration is acquired after `HWND` creation, before WebView2 acquisition or visibility, and is released after controller cleanup but before native-window destruction.

The native frame and WebView2 profile use the same portable preference. Explicit mutations run on the owning STA, update the native frame before the profile, roll the frame back if the profile update fails, and publish `INantoApplicationHost.PreferredColorScheme` only after both succeed. `ColorValuesChanged` marshals to the STA and reapplies the frame only in `System`; explicit `Light` or `Dark` ignores later system notifications. WebView2 remains in its native `Auto` mode for `System`, so the SPA follows through `prefers-color-scheme` without a Nanto message. Fast tests cover preference resolution, narrow-ABI activation/release, callback suppression and failure containment, DWM attribute mutation, system-event behavior, detached-window races, acquisition failure, and zero ledger counts. Hidden production tests retain live Dark/Light/System SPA verification and include apartment, activation, interface, callback, and subscription checkpoints in the fault/cancellation matrix.

Renderer recovery projects only `ProcessFailed`, `ProcessFailedKind`, `Reload`, and the completion callback used by the hidden crash seam beyond the existing WebView2 vtable closure. Subscribe before mapping and initial navigation, register the subscription immediately, and remove it before the other WebView subscriptions. Map browser and main-renderer exits to portable `Exited`, renderer stalls to `Unresponsive`, frame-only exits to `FrameRendererExited`, and other child-process exits to `Unknown`; never expose WebView2 enums publicly. Raise the portable event synchronously on the owning STA before taking action, isolate exceptions from application handlers, and timestamp it with the window's injected clock.

The first main-frame renderer exit or unresponsive notification in a window lifetime reports `WillAttemptRecovery = true` and starts one in-place reload only while the window is running and no close has been requested. Successful recovery is the following successful navigation completion. If reload initiation throws, that navigation fails, another main-renderer failure arrives, or the window is already closing, request ordinary window closure exactly once. Browser-process exit reports no recovery and closes because WebView2 requires recreation; after that exit, teardown releases Nanto's logical ownership without invoking WebView2 mutation methods against unavailable native state. Phase 1 does not add controller/environment recreation. Frame-only and utility, sandbox, GPU, plugin, or unknown child-process exits report without reload or closure because WebView2 isolates or recreates those processes. The hidden production fixture uses internal test seams—not public API—to invoke Chromium's `Page.crash` and terminate the isolated browser process, proving renderer reload, browser-loss closure, subscription cleanup, normal shutdown, and zero final ledgers. Fast tests cover the complete decision table, one-shot deduplication, close races, failed navigation, reload failure, portable event timestamping, and handler isolation.
...
Run `dotnet test tests/Nanto.Hosting.Windows.LongRunningIntegrationTests/Nanto.Hosting.Windows.LongRunningIntegrationTests.csproj -p:TestScope=All -p:RunManualTests=true` separately only when explicitly approving its machine time. A 45-minute attempt completed 397 of 550 processes cleanly and deleted the shared application root before the former deadline expired. The complete August 23, 2026 rerun passed all 550 processes within the measured 75-minute deadline and deleted the shared application root. The gate record now keeps only mixed-DPI visible acceptance open; it remains acceptance debt rather than unfinished production implementation.
