...
Phase 1, Phase 2, and Phase 3 implementation are closed, and [`docs/phase4-plan.md`](docs/phase4-plan.md) governs the current capability and plugin-foundation work. Preserve the completed Windows host, lifecycle, asset/navigation, appearance, diagnostics, deployment, versioned protocol, generated-contract, authorization, frontend bridge, CLI, SDK, template, development-loop, telemetry, and optional Aspire guarantees. The complete 550-process lifecycle/recovery soak passed on August 23, 2026. One Phase 1 acceptance follow-up remains open: the visible cross-monitor run on two active monitors with different effective DPI. Keep that item visible in the gate record; do not describe it as passed. Nanto must remain self-contained and must not reference or copy experimental assemblies. Phase 4 may package its capability tooling, static plugin contracts, generated registries, and fixtures for isolated installation tests, but avoid application installers/distribution packaging, the Phase 5 production plugin catalogue, a mandatory DI container, multi-window support, polished UI, and future-platform work until their roadmap phases.
